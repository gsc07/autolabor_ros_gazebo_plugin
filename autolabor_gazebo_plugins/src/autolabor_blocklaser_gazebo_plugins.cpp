...
#include <autolabor_gazebo_plugins/autolabor_blocklaser_gazebo_plugins.h>
...
// Register this plugin with the simulator
    GZ_REGISTER_SENSOR_PLUGIN(BlockLaserPlugin)

////////////////////////////////////////////////////////////////////////////////
// Constructor
    BlockLaserPlugin::BlockLaserPlugin() {
    }
...
////////////////////////////////////////////////////////////////////////////////
// Load the controller
    void BlockLaserPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) {
        // load plugin
        ...
        if (!this->parent_ray_sensor_) gzthrow("GazeboRosBlockLaser controller requires a Ray Sensor as its parent");
        ...
        if (!ros::isInitialized()) {
            ROS_FATAL_STREAM_NAMED("block_laser",
        ...
        if (this->topic_name_ != "") {
            ...
            ros::AdvertiseOptions ao = ros::AdvertiseOptions::create<sensor_msgs::PointCloud2>(
                    this->topic_name_, 1,
                    boost::bind(&BlockLaserPlugin::LaserConnect, this),
                    boost::bind(&BlockLaserPlugin::LaserDisconnect, this), ros::VoidPtr(), &this->laser_queue_);
        ...
        this->callback_laser_queue_thread_ = boost::thread(boost::bind(&BlockLaserPlugin::LaserQueueThread, this));

    }

////////////////////////////////////////////////////////////////////////////////
// Increment count
    void BlockLaserPlugin::LaserConnect() {
...
////////////////////////////////////////////////////////////////////////////////
// Decrement count
    void BlockLaserPlugin::LaserDisconnect() {
        this->laser_connect_count_--;
...
// Update the controller
    void BlockLaserPlugin::OnNewLaserFrame(const float *_image,
...
// Put laser data to the interface
    void BlockLaserPlugin::PutLaserData(common::Time &_updateTime) {
        int i, hja, hjb;
        int j, vja, vjb;
        double vb, hb;
...
//                    this->cloud_msg_.points.push_back(point);
        ...
        this->parent_ray_sensor_->SetActive(true);
        sensor_msgs::convertPointCloudToPointCloud2(this->cloud_msg_, this->cloud_msg2_);
...
// Utility for adding noise
    double BlockLaserPlugin::GaussianKernel(double mu, double sigma) {
...
// custom callback queue thread
    void BlockLaserPlugin::LaserQueueThread() {
    ...
    void BlockLaserPlugin::OnStats(const boost::shared_ptr<msgs::WorldStatistics const> &_msg) {
